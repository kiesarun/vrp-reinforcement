...
class Simulator:
    ...
    def get_max_variance_car(self):
        ...
        for i, car in enumerate(self.cars):
            car_volume = scale_volume(car.volume)
            variance = (car_volume - mean) ** 2
            if variance > max_variance:
                max_variance = variance
                max_variance_car = i
        return max_variance_car
    ...
    def get_state(self):
        ...
        if volume_std > VOLUME_STD:
            state_string = state_string + '1'
            print('standard_volume :  ', volume_std, '>', VOLUME_STD)
        else:
        ...
        if state_string == '021':
            state = 0
        elif state_string == '020':
            state = 1
        elif state_string == '011':
            state = 2
        elif state_string == '010':
            state = 3
        elif state_string == '001':
            state = 4
        elif state_string == '000':
            state = 5
        elif state_string == '121':
            state = 6
        elif state_string == '120':
            state = 7
        elif state_string == '111':
            state = 8
        elif state_string == '110':
            state = 9
        elif state_string == '101':
            state = 10
        elif state_string == '100':
            state = 11
        elif state_string == '221':
            state = 12
        elif state_string == '220':
            state = 13
        elif state_string == '211':
            state = 14
        elif state_string == '210':
            state = 15
        elif state_string == '201':
            state = 16
        else:
            state = 17
        return state
    ...
    def take_action(self, action):
        if self.is_train:
            ...
            if action == 0:
                ...
                self.move_from_max_variance_to_nearest()
            else:
                self.delete_car()
    ...
    def get_reward(self, pre_delivery_cars, pre_not_full_cars):
        ...
        std_volume = self.get_std_deviation_volume()

        finish_rate = self.get_finish_cars() / len(self.cars)
        reward = (diff_delivery + diff_not_full + (finish_rate * 2)) - std_volume
        return reward
    ...
    def move_farthest_order_from_most_orders_to_nearest_car(self):
        if self.is_train:
            if len(self.cars) > 1:
                pre_delivery_cars = self.can_delivery_cars
                pre_not_full_cars = self.not_excess_cars
                pre_std_volume = self.get_std_deviation_volume()
    ...
    def move_from_max_variance_to_nearest(self):
        if self.is_train:
            if len(self.cars) > 1:
                pre_delivery_cars = self.can_delivery_cars
                pre_not_full_cars = self.not_excess_cars
                ...
                max_variance_car = self.get_max_variance_car()
                farthest_order = self.find_farthest_order(max_variance_car)
                lat = self.cars[max_variance_car].orders[farthest_order].coordinate['lat']
                lon = self.cars[max_variance_car].orders[farthest_order].coordinate['lon']
                nearest_car, nearest_orders = self.find_nearest_car_and_order(lat, lon, max_variance_car)
                if nearest_car != max_variance_car:
                    self.move_order(max_variance_car, nearest_car, farthest_order)
